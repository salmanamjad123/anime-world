/**
 * Manga Profile Page
 * Top-level tabs: Profile | Continue Reading | Read List
 */
...
const SUB_TABS: { key: 'all' | ListStatus; label: string }[] = [
  { key: 'all', label: 'All' },
  { key: 'watching', label: 'Reading' },
  { key: 'on-hold', label: 'On-Hold' },
  { key: 'plan-to-watch', label: 'Plan to read' },
...
function ReadListCard({
  ...
  item: { mangaId: string; title: string; image: string; addedAt: Date; status?: ListStatus };
  ...
  return (
    <div ref={ref} className="relative group bg-gray-800 rounded-lg overflow-visible hover:ring-2 hover:ring-amber-500 transition-all">
      <Link href={ROUTES.MANGA_DETAIL(item.mangaId)} className="block overflow-hidden rounded-lg">
        ...
        <div className="absolute top-10 left-2 right-2 sm:left-auto sm:right-2 sm:w-44 z-50 py-1 rounded-lg bg-gray-800 border border-gray-700 shadow-xl min-w-[10rem]">
          {LIST_OPTIONS.map((opt) => {
            ...
            return (
              ...
              >
                {isSelected ? <Check className="w-4 h-4 text-amber-400 shrink-0" /> : <span className="w-4 shrink-0" />}
                {opt.label}
              </button>
            );
          })}
...
function MangaProfilePageContent() {
  ...
  const activeSubTab: 'all' | ListStatus =
    activeSection === 'readlist' && tabParam && SUB_TABS.some((t) => t.key === tabParam)
  ...
  const setSection = (s: Section) => {
    ...
    if (s === 'readlist') params.set('tab', 'all');
    else params.delete('tab');
    router.replace(`${ROUTES.MANGA_PROFILE}?${params.toString()}`, { scroll: false });
  };

  const setSubTab = (t: 'all' | ListStatus) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set('section', 'readlist');
    params.set('tab', t);
    router.replace(`${ROUTES.MANGA_PROFILE}?${params.toString()}`, { scroll: false });
  };
  ...
  useEffect(() => {
    if (!user && !isLoading) router.replace(ROUTES.MANGA);
      ...
      router.replace(ROUTES.MANGA);
      openAuthModal('verify');
    }
  }, [user, isLoading, router, openAuthModal]);

  const itemsByTab = useMemo(() => {
    const map: Record<ListStatus, typeof readlist> = {
      watching: [],
      'on-hold': [],
      'plan-to-watch': [],
      dropped: [],
    ...
    return map;
  }, [readlist, getItemsByStatus]);

  const allItems = useMemo(() => [...readlist], [readlist]);
  ...
  const renderGrid = (items: typeof readlist) => (
      ...
      {items.map((item) => (
        <ReadListCard
          ...
          onStatusChange={async (status) => {
            setListStatus(item.mangaId, status);
            if (user?.uid) {
              try {
                await setMangaListItem(user.uid, item.mangaId, item.title, item.image, status);
              } catch (e) {
          ...
          onRemove={async () => {
            removeFromList(item.mangaId);
            if (user?.uid) {
              try {
                await removeFromMangaList(user.uid, item.mangaId);
              } catch (e) {
  ...
  const renderReadingWithProgress = () => {
    if (continueReading.length === 0) {
      return renderEmpty('You are not reading any manga right now.', {
    ...
    const handleClearAll = async () => {
      if (!confirm('Clear all reading progress?')) return;
      clearHistory();
      if (user?.uid) {
        try {
      ...
      <>
        {user && continueReading.length > 0 && (
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm text-gray-400">{continueReading.length} in progress</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {continueReading.map((item) => {
            const readUrl =
              ROUTES.MANGA_READ(item.mangaId, item.chapterId, item.provider) +
              ...
              >
                <Link href={readUrl} className="flex gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0 py-1 pr-10">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">{item.percentage}% read</p>
                  </div>
                </Link>
                {user && (
                  <button
    ...
    if (!user) {
      return (
        <section className="text-center py-16">
          ...
          <Button variant="primary" onClick={() => openAuthModal()} className="bg-amber-600 hover:bg-amber-700">
            Sign In
          </Button>
        </section>
      );
      ...
      <section className="flex flex-col lg:flex-row gap-8">
        <div className="flex-1 space-y-6 order-2 lg:order-1">
              ...
              <div className="mt-3 p-4 rounded-lg bg-gray-800 border border-gray-700 flex items-start gap-3">
                ...
                <div>
                  ...
                  <button
                    ...
                    className="text-sm text-amber-400 hover:text-amber-300 underline mt-1 disabled:opacity-50"
                  >
          ...
          {saveSuccess && (
            <p className="text-sm text-amber-400">
              Changes saved. Check your email if you requested verification or password reset.
            </p>
          )}
          ...
          <div className="pt-6 mt-6 border-t border-gray-800">
            <Button
              ...
              className="w-full sm:w-auto min-w-[140px] px-10 py-3 text-base font-semibold bg-amber-600 hover:bg-amber-700"
            >
              Save
            </Button>
          </div>
        ...
        <div className="lg:w-48 shrink-0 flex flex-col items-center order-first lg:order-none">
          <div className="relative">
            ...
            >
              {photoLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Pencil className="w-4 h-4" />}
            </button>
          </div>
        </div>
      </section>
  ...
  const displayItems = activeSubTab === 'all' ? allItems : itemsByTab[activeSubTab] ?? [];
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <main className="container mx-auto px-4 py-8">
        {(!user || !user.emailVerified) && !isLoading ? null : isLoading ? (
          ...
          <>
            <div className="mb-6 rounded-xl overflow-hidden bg-gradient-to-r from-amber-600/20 via-amber-500/10 to-transparent border border-gray-800/50">
              ...
              <nav className="flex gap-0.5 sm:gap-1 overflow-x-auto">
                {TOP_TABS.map(({ key, label, icon: Icon }) => {
                  ...
                  return (
                    <Link
                      key={key}
                      href={ROUTES.MANGA_PROFILE_SECTION(key)}
                      onClick={(e) => {
                        e.preventDefault();
                        setSection(key);
                      }}
                    ...
                    >
                      <Icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 ${active ? 'text-amber-400' : ''}`} />
                      {label}
                      {active && (
                        <span className="absolute left-2 right-2 bottom-0 h-0.5 bg-amber-500 rounded-full" />
                      )}
                    </Link>
                  );
                })}
            ...
            {activeSection === 'profile' && <section>{renderProfileSection()}</section>}
            ...
            {activeSection === 'readlist' && (
              <section>
                {displayItems.length === 0
                  ? renderEmpty(
                      activeSubTab === 'all'
                        ? 'Your list is empty. Add manga from any manga page.'
                        : `No manga in ${SUB_TABS.find((t) => t.key === activeSubTab)?.label ?? activeSubTab}.`,
                      { label: 'Browse manga', href: ROUTES.MANGA }
                    )
...
export default function MangaProfilePage() {
  return (
    <Suspense
      fallback={
